...
<body>
    <div class="header">
        <h1>Relatório de Vendas - Hospedagem</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            {{-- CORREÇÃO AQUI: Usando a variável correta --}}
            @foreach($vendasReservasAgrupadas as $nomeVendedor => $reservas)
                <tr class="row-vendedor"><td colspan="6">{{ $nomeVendedor }}</td></tr>

                @php $subtotal = 0; @endphp

                @foreach($reservas as $reserva)
                    @php $subtotal += $reserva->valor_total; $totalGeral += $reserva->valor_total; @endphp
                    <tr>
