...
<body>
    <div class="header">
        ...
        <p>Período: {{ \Carbon\Carbon::parse($dataInicio)->format('d/m/Y') }} a
            {{ \Carbon\Carbon::parse($dataFim)->format('d/m/Y') }}</p>
    </div>
                ...
                @foreach ($dayUses as $du)
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            {{ isset($du->data) ? \Carbon\Carbon::parse($du->data)->format('d/m/Y') : '-' }}</td>
                        <td class="text-right">R$ {{ number_format($valor, 2, ',', '.') }}</td>
                    </tr>
                @endforeach
                <tr class="row-subtotal">
                    <td colspan="3" class="text-right">Total {{ $nomeVendedor }}:</td>
