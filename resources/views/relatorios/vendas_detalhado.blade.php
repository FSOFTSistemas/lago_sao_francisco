                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        @foreach($vendasReservasAgrupadas as $nomeVendedor => $reservas)
                            <tr class="bg-secondary"><td colspan="6" class="pl-3 font-weight-bold">{{ $nomeVendedor }}</td></tr>
                            @php $sub = 0; @endphp
                            @foreach($reservas as $reserva)
                                @php $sub += $reserva->valor_total; $totalGeralReserva += $reserva->valor_total; @endphp
                                <tr>
                            ...
                            @endforeach
                            <tr style="background-color: #f1f1f1;"><td colspan="5" class="text-right font-weight-bold">Total {{ $nomeVendedor }}:</td><td class="text-right font-weight-bold">R$ {{ number_format($sub, 2, ',', '.') }}</td></tr>
                        @endforeach
                    </tbody>
                    @if($vendedorId == 'todos')
                    <tfoot><tr class="bg-dark font-weight-bold"><td colspan="5" class="text-right">TOTAL GERAL RESERVAS:</td><td class="text-right">R$ {{ number_format($totalGeralReserva, 2, ',', '.') }}</td></tr></tfoot>
                    @endif
                </table>
            </div>
            @endif
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        @foreach($vendasDayUseAgrupadas as $nomeVendedor => $dayUses)
                            <tr class="bg-secondary"><td colspan="4" class="pl-3 font-weight-bold">{{ $nomeVendedor }}</td></tr>
                            @php $sub = 0; @endphp
                            @foreach($dayUses as $du)
                                @php 
                                    $valor = $du->total ?? $du->valor_total ?? $du->valor ?? 0; 
                                    $sub += $valor; $totalGeralDayUse += $valor; 
                                    $cliente = $du->hospede->nome ?? $du->cliente_nome ?? $du->nome_cliente ?? 'N/D';
                                    $dataUso = isset($du->data) ? \Carbon\Carbon::parse($du->data)->format('d/m/Y') : '-';
                                @endphp
                                <tr>
                            ...
                            @endforeach
                            <tr style="background-color: #f1f1f1;"><td colspan="3" class="text-right font-weight-bold">Total {{ $nomeVendedor }}:</td><td class="text-right font-weight-bold">R$ {{ number_format($sub, 2, ',', '.') }}</td></tr>
                        @endforeach
                    </tbody>
                    @if($vendedorId == 'todos')
                    <tfoot><tr class="bg-success text-white font-weight-bold"><td colspan="3" class="text-right">TOTAL GERAL DAY USE:</td><td class="text-right">R$ {{ number_format($totalGeralDayUse, 2, ',', '.') }}</td></tr></tfoot>
                    @endif
                </table>
            </div>
            @endif
