<form wire:submit.prevent="savePayments">
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            ...
            <select id="paymentMethod" class="form-select" wire:model="selectedPaymentMethodId">
                <option value="">Selecione</option>
                @foreach($formaPagamento as $metodo)
                    <option value="{{ $metodo->id }}">{{ $metodo->descricao }}</option>
                @endforeach
            </select>
            @error('selectedPaymentMethodId') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
        <div class="col-md-4">
    ...
    @if(count($currentPayments) > 0)
        <ul class="list-group mb-3">
            @foreach($currentPayments as $index => $payment)
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    {{ $payment['descricao'] }}: R$ {{ number_format($payment['valor'], 2, ',', '.') }}
    ...
    <div class="row mt-4">
        <div class="col-12 text-center">
            <label for="remainingToPayField" class="form-label fs-3">Restante a Pagar</label>
            <input type="text" id="remainingToPayField" class="form-control form-control-lg text-center {{ $remainingToPay > 0 ? 'text-danger' : 'text-success' }} fw-bold" value="R$ {{ number_format($remainingToPay, 2, ',', '.') }}" readonly>
            @error('remainingToPay') <span class="text-danger">{{ $message }}</span> @enderror
        </div>
    </div>
