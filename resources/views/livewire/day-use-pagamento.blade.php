<form wire:submit.prevent="savePayments">
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-3">
            ...
            <input type="number" min="1" class="form-control" wire:model.lazy="souvenirQuantidade"
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="text" id="subtotalItems" class="form-control"
                value="R$ {{ number_format($itemSubtotal, 2, ',', '.') }}" readonly>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="number" step="0.01" class="form-control" id="acrescimo"
                wire:model.live.debounce.1000ms="acrescimo" min="0">
            @error('acrescimo')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="number" step="0.01" class="form-control" id="desconto"
                wire:model.live.debounce.1000ms="desconto" min="0">
            @error('desconto')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="text" id="finalTotal" class="form-control form-control-lg text-success fw-bold"
                value="R$ {{ number_format($finalPagamentoTotal, 2, ',', '.') }}" readonly>
        </div>
    </div>
    ...
    <div class="row form-group mb-3 ">
        <div class="col-md-6">
            ...
            @error('metodoSelecionadoID')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
        <div class="col-md-4">
            <label for="pagamentoValor" class="form-label">Valor</label>
            <input type="number" step="0.01" class="form-control" id="pagamentoValor"
                wire:model.live="pagamentoValor" min="0.01" {{ $restante == 0 ? 'readonly' : '' }}>
            <div class="text-danger" style="min-height: 20px;">
                @error('pagamentoValor')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
        </div>
            ...
            @foreach ($pagamentosAtuais as $index => $pagamento)
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-danger btn-sm"
                        wire:click="removePayment({{ $index }})">Remover</button>
                </li>
            @endforeach
        </ul>
    @else
    ...
    <div class="row mt-4">
        <div class="col-12 text-center">
            ...
            <input type="text" id="restanteField"
                class="form-control form-control-lg text-center {{ $restante > 0 ? 'text-danger' : 'text-success' }} fw-bold"
                value="R$ {{ number_format($restante, 2, ',', '.') }}" readonly>
            @error('restante')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
