...
use App\Models\ContasAReceber; // Garanta que o nome do Model está correto
...
class PlanoDeContasService
{
    ...
    private function calcularTotaisIndividuais(?int $empresaId = null, $dataInicio = null, $dataFim = null): array
    {
        $totais = [];
        ...
        $fluxoQuery = FluxoCaixa::query()
            ...
            ->select('fluxo_caixas.plano_de_conta_id', DB::raw("SUM(CASE WHEN fluxo_caixas.tipo = 'entrada' THEN fluxo_caixas.valor WHEN fluxo_caixas.tipo = 'saida' THEN -fluxo_caixas.valor WHEN fluxo_caixas.tipo = 'cancelamento' THEN -ABS(fluxo_caixas.valor) ELSE 0 END) as total"))
        ...
        if ($dataInicio && $dataFim) {
            $fluxoQuery->whereBetween('fluxo_caixas.data', [$dataInicio, $dataFim]);
        }
        ...
        $parcelaQuery = ParcelaContasAPagar::query()
            ...
            ->whereIn('parcelas_contas_a_pagar.status', ['pago', 'finalizado'])
        ...
        if ($dataInicio && $dataFim) {
            $parcelaQuery->whereBetween('parcelas_contas_a_pagar.data_pagamento', [$dataInicio, $dataFim]);
        }
        ...
        $receberQuery = ContasAReceber::query()
            ...
            ->whereIn('status', ['recebido', 'finalizado'])->whereNotNull('plano_de_contas_id')->groupBy('plano_de_contas_id');
